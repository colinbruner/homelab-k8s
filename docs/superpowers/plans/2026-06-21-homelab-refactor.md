...
- [ ] **Step 3 — monitoring: DEFERRED (user decision 2026-06-22).** Monitoring is currently archived/not-running, and the spec's "standalone prometheus-operator" chart is deprecated upstream. Decision: do NOT build `platform/monitoring/` in this refactor PR; reintroduce monitoring as its own focused follow-up PR. `k8s/archived/monitoring/` is RETAINED (not deleted in Task 14) as the basis for that work. The `platform` ApplicationSet's wave map keeps a `monitoring:0` entry harmlessly (no dir = no Application generated).

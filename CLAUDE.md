...
- **`k8s/platform/`** — ArgoCD-managed infrastructure, ordered by sync-wave annotations (1password -30, metallb -20, cert-manager -15, gateway -10, csi-nfs -5, storage 0).
...
- **`packages/helm/`** — Local Helm charts (postgres, kopia) consumed by Kustomize `helmCharts:` with `helmGlobals.chartHome`.
...
5. **Internal DNS** — add a `foo-internal` A record (MetalLB Gateway IP) via the Terraform Cloudflare config and apply
...
`<name>-internal.colinbruner.com` A records point at the MetalLB Gateway IP.
Managed via Terraform (outside this repo; not GitOps) — apply with the Terraform Cloudflare workflow.

**Public CNAME records** (Cloudflare-managed):
Created via `cloudflared tunnel route dns` CLI. Tunnel CNAME records are outside GitOps.
